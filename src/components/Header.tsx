...
export default function Header() {
    ...
    const date = useAppSelector(todosSlice.selectors.selectSelectListDate)
    const numberToDate = (date: number) => new Date(date).toISOString().split("T")[0]
    ...
    function handleDateChange(e: React.ChangeEvent<HTMLInputElement>) {
        const dateNumber: number = new Date(e.target.value).getTime()
        const list: TodoList | undefined = Object.values(store.getState().todos.lists).find(
            l => l.date === dateNumber)
        if (list === undefined) {
            const days = (dateNumber - store.getState().todos.lists[0].date) / 86400000
            dispatch(todosSlice.actions.newList({listId: days, date: dateNumber}))
            dispatch(todosSlice.actions.selected({listId: days}));
        }
        else {
            dispatch(todosSlice.actions.selected({listId: list.id}))
        }
    ...
    return (
        <header className={styles.header}>
            ...
            <div className={styles.header_center}>
                <ArrowButton />
                <input type="date" value={numberToDate(date)} onChange={(e) => handleDateChange(e)}
