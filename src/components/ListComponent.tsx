...
import {selectTheme} from "../store.ts";
        ...
        <main className={styles.main}>
            ...
            <form className={styles.add_todo_main} onSubmit={handleSubmit}>
                <div className={styles2.todo}>
                    <input ref={textInputRef} className={`${styles.add_todo_text} ${styles.todo_input}`} type='text' value={todoName} placeholder="Enter To-Do name"
                           onChange={(e) => setTodoName(e.target.value)}/>
                    <div className={styles.add_time_container}>
                        <input className={`${styles.add_time_start} ${styles.todo_input}`} type='time'
                               value={Date()}></input>
                        <input className={`${styles.add_time_end} ${styles.todo_input}`} type='time'
                               value={Date()}></input>
                    </div>
                </div>
            ...
            <div className={styles.todo_list}>
                {todos.map((s, key) => (<TodoComponent key={key} todoText={s}/>))}
            </div>
        </main>
    )
}